...
def make_monitor(w: int = 1000, h: int = 800, tags_mask: int = 1) -> wl.Monitor:
    m = wl.Monitor(wlr_output=MagicMock(), scene_output=MagicMock(), name="test")
    m.m = (0, 0, w, h)
    m.w = (0, 0, w, h)
...
def make_client(
    ...
    return wl.Client(
        client_type=wl.ClientType.XdgShell,
...
def test_commit_reapplies_clip(monkeypatch):
    ...
    c.xdg_toplevel.base.initial_commit = False
    c.xdg_toplevel.base.geometry.x = 10
    c.xdg_toplevel.base.geometry.y = 20
...
def test_commit_initial_no_resize():
    ...
    c.geometry = (0, 0, 1000, 800)
    s.clients.append(c)
    c.xdg_toplevel.base.initial_commit = True
    wl.lib.wlr_xdg_toplevel_set_size.reset_mock()
...
def test_resize_short_circuits_when_dims_match():
    ...
    c.xdg_toplevel.current.width = 1000 - 2 * bw
    c.xdg_toplevel.current.height = 800 - 2 * bw
    wl.lib.wlr_xdg_toplevel_set_size.reset_mock()
...
def test_resize_calls_set_size_when_dims_differ():
    ...
    c.geometry = (0, 0, 1000, 800)
    s.clients.append(c)
    c.xdg_toplevel.current.width = 0   # differs from any inner box
    c.xdg_toplevel.current.height = 0
